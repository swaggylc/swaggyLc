...
export const useMealsStore = defineStore('meals', {
    ...
    getters: {
        ...
        totalCount: (state) => {
            // 如果购物车中没有商品直接返回0
            if (state.carMeals.length <= 0) return 0;
            // 有商品，计算总数量
            return state.carMeals.reduce((result, item) => result + item.count, 0);
        },
        // 获取购物车中商品的总价格
        totalMenoy: (state) => {
            // 如果购物车中没有商品直接返回0
            if (state.carMeals.length <= 0) return 0;
            // 有商品，计算总价格
            return state.carMeals.reduce((result, item) => result + item.count * item.price, 0);
        }
    },
    actions: {
        addMealToCar(meal) {
